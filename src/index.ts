...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.GuildMembers, // Essential for adding users to threads
    ],
});
...
if (!token) {
    ...
    console.log(`⏳ Attempting Discord Handshake... (Token Length: ${token.length})`);
...
client.once('ready', () => {
    console.log(`🚀 SUCCESS: ${client.user?.tag} is online and watching for matches!`);
});

// 4. REGISTER SLASH COMMANDS (Runs in parallel)
const commands = [
...
client.on('interactionCreate', async (interaction: Interaction) => {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'setup') {
        const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
            new ButtonBuilder()
                ...
                .setLabel('💬 Chat with random person')
                .setStyle(ButtonStyle.Primary)
        );
        await interaction.reply({ 
            content: '**Welcome to Strangers Chat!**\nClick the button below to find a match. Privacy is guaranteed.', 
            components: [row] 
        });
        return;
    }
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'leave') {
        ...
        if (queueData) {
            ...
            return interaction.editReply('You have left the queue.');
        }

        // Check if in active chat
        const { data: chatData } = await supabase.from('active_chats')
            .select('*')
        ...
        if (chatData) {
            ...
            if (sharedThread) {
                await sharedThread.send('🔴 **A user has left the chat. Closing room...**');
                setTimeout(() => sharedThread.delete().catch(() => {}), 5000);
            }
            return interaction.editReply('You have left the chat.');
        }
        return interaction.editReply('You are not currently in a chat or queue!');
    }
    ...
    if (interaction.isButton() && interaction.customId === 'join_queue') {
        ...
        if (waitingUser) {
            ...
            const thread = await channel.threads.create({ 
                name: `Private Chat Queue`, 
                type: ChannelType.PrivateThread 
            });
            await thread.members.add(userId);
            await supabase.from('queue').insert({ discord_id: userId, thread_id: thread.id });
            await thread.send(`Hey 👋 <@${userId}>, I'm looking for a match for you. Please wait... 👀`);
            return interaction.editReply(`Joined queue! I will ping you here when a match is found: <#${thread.id}>`);
        }
    }
});

// 6. ANTI-SLEEP WEB SERVER (For Render)
const express = require('express');
const server = express();
server.get('/', (req: any, res: any) => res.send('Matchbot is Online!'));
